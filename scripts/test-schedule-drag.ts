...
async function main() {
  ...
  await page.evaluate(() => {
    ...
    const ds = new DragEvent("dragstart", {
      bubbles: true,
      cancelable: true,
      dataTransfer: dt,
    });
    ...
    const de = new DragEvent("dragover", {
      bubbles: true,
      cancelable: true,
      dataTransfer: dt,
    });
  ...
  const expected =
    ...
    windows[1].start === "18:00" &&
    windows[2].start === "02:00";
  console.log(
    expected ? "PASS: Reordered correctly" : "FAIL: Order did not change",
  );
